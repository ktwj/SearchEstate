...
def searching(station, mins, minp, maxp):
    t = f"and_(or_(and_(search_list.station1.like(f'%{station}%'), search_list.time1 <= mins), and_(search_list.station2.like(f'%{station}%'), search_list.time2 <= mins), and_(search_list.station3.like(f'%{station}%'), search_list.time3 <= mins)), {minp} <= search_list.price, search_list.price <= {maxp})"
