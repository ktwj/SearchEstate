...
def searching(station, mins, minp, maxp, shikirei, room_size):
    ...
    t = text(f"( (station1 like '%{station}%' and time1<={mins}) or (station2 like '%{station}%' and time2<={mins}) or (station3 like '%{station}%' and time3<={mins}) ) and {minp}<=price and price<={maxp} and deposit+key <= {shikirei} and room_size >= {room_size}")
